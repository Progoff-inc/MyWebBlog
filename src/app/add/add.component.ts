...
export class AddComponent implements OnInit {
  ...
  addTask(){
    ...
    this.dv.AddTask({Name:this.taskForm.value.Name, Description:this.taskForm.value.Description, RequirementId:this.taskForm.value.RequirementId, UserId:this.taskForm.value.UserId, Priority:this.taskForm.value.Priority, Status:Status.Proposed, ProjectId:this.ProjectId}).subscribe(data => {
  ...
  addReq(){
    ...
    this.dv.AddRequirement({Name:this.reqForm.value.Name, Description:this.reqForm.value.Description, ProjectId:this.ProjectId, Status:Status.Proposed}).subscribe(data => {
      this.router.navigate(
          ['/projects', this.ProjectId]
      );
    })
